import { cardscrollRight, scrollUp } from "../animations/effects";
    ...
    return(
        <>
            <div className="w-full p-5 flex flex-col items-center bg-[#F9FAFB] overflow-hidden">
                <motion.h3 {...scrollUp} className="text-3xl font-bold mt-5">
                    Meet Our Doctors
                </motion.h3>
                <div className="w-full mt-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 p-5 overflow-hidden">
                    {doctors.map((doctor) =>(
                        <motion.div {...cardscrollRight} key={doctor.id} className="mb-5 lg:mb-0">
                            <img src={doctor.image} className="w-full h-50 object-cover object-top rounded-t-md"/>
