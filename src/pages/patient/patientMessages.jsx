                    ...
                    {conversations.length === 0 ? (
                        ...
                        conversations.map((conv) => (
                            ...
                            >
                                {/* Avatar */}
                                {conv.doctor_picture ? (
                                    <img src={conv.doctor_picture} className='w-10 h-10 rounded-full object-cover flex-shrink-0' alt="" />
                                ) : (
                                ...
                                <div className='flex-1 min-w-0'>
                                    <div className='flex justify-between items-center'>
                                        <p className='text-sm font-semibold text-slate-800 truncate'>{conv.doctor_name}</p>
                                        {conv.unread_count > 0 && (
                                    ...
                                    <p className='text-[10px] font-medium text-slate-500'>{conv.doctor_specialization}</p>
                                    {conv.last_message && (
                ...
                {activeConv ? (
                    <>
                        ...
                        <div className='bg-white px-5 py-3.5 border-b border-slate-100 flex items-center space-x-3'>
                            {activeConv.doctor_picture ? (
                                <img src={activeConv.doctor_picture} className='w-9 h-9 rounded-full object-cover' alt="" />
                            ) : (
                                <div className='w-9 h-9 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center text-xs font-bold'>
                                    {getInitials(activeConv.doctor_name)}
                                </div>
                            )}
                            <div>
                                <p className='text-sm font-semibold text-slate-800'>{activeConv.doctor_name}</p>
                                <p className='text-xs text-slate-400'>{activeConv.doctor_specialization}</p>
                            </div>
                        </div>
