    ...
    return(
        <>
            ...
            <div className="bg-[#F9FAFB] w-full flex flex-col p-5 mt-5 items-center overflow-hidden">
                ...
                <div className="w-full mt-5 lg:px-10 py-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 overflow-hidden" id="services">
                    {services.map((service)=>(
                        <motion.div {...cardscrollRight} className="rounded-md bg-white mb-3 overflow-hidden">
                            <img src={service.image} className="w-full h-55 object-cover rounded-t-md"/>
                            <div className="flex flex-col p-4 rounded-b-md bg-white shadow-xs">
                                ...
                                <div className="flex space-x-4 my-3">
                                    <motion.button {...buttonEffects} className="px-7 rounded-sm cursor-pointer bg-blue-600 text-white py-2 text-xs font-semibold">
                                        <a href="#addServices">
                                            Learn More
                                        </a>
                                    </motion.button>
                                    <motion.button {...buttonEffects} className="border-1 border-blue-600 px-7 rounded-sm cursor-pointer text-blue-600 py-2 text-xs font-semibold">
            ...
            <div className="w-full flex flex-col items-center justify-center space-y-3 bg-red-500 py-12 overflow-hidden">
                ...
                <p className="text-xs md:text-sm text-gray-200 w-90 md:w-150 text-center">
            ...
            <div className="w-full flex flex-col items-center p-5 mt-5 overflow-hidden" id="addServices">
                <motion.h3 {...scrollRight} className="text-2xl font-bold mt-5">
                    Comprehensive Care Packages
                </motion.h3>
                <motion.p {...scrollUp} className="text-gray-500 w-90 md:w-150 text-xs md:text-[14px] mt-2 text-center">
                    Specially designed care packages that combine multiple services for comprehensive treatment and better outcomes.
                </motion.p>
                <div className="flex w-full mt-5 lg:px-10 py-5 grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 overflow-hidden">
                    {packages.map((data) => (
                        <motion.div {...cardscrollRight} key={data.id} className={`flex flex-col items-left space-y-3 p-4 rounded-sm ${data.card_bg} overflow-hidden`}>
                            ...
                            {data.activities.map((datas)=>(
                                <p className="text-xs flex items-center"><FaCheck  className={`w-2 h-2 mt-1 mr-1 ${data.price_style}`}/>{datas}</p>
                            ))}
            ...
            <div className="w-full p-5 lg:flex lg:flex-row lg:justify-center lg:items-center mt-5 oveflow-hidden" id="technology">
